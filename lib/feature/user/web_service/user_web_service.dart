...
abstract class UserBaseWebServices {
  ...
  Future<FullCountryEntity> getFullCountry(
      GetFullCountryParams getFullCountryParams);
...
class UserWebServicesImpl implements UserBaseWebServices {
  ...
  @override
  Future<FullCountryEntity> getFullCountry(
      GetFullCountryParams getFullCountryParams) async {
    final response = await _apiConsumer.post(EndPoints.getFullCountry,
        queryParameters: getFullCountryParams.toJson());
    return FullCountryEntity.fromJson(response);
  }

  @override
  Future<FullCityEntity> getFullCity(
      GetFullCityParams getFullCityParams) async {
    final response = await _apiConsumer.post(EndPoints.getFullCountry,
        queryParameters: getFullCityParams.toJson());
    return FullCityEntity.fromJson(response);
  }

  Future<FormData> formAddphoto(
      AddPhotoToProductParams addPhotoToProductParams) async {
  ...
  @override
  Future<BaseEntity> addPhotoToProduct(
      AddPhotoToProductParams addPhotoToProductParams) async {
